...
def load(folder):
    tr = pd.read_csv(f"{folder}/feedback_train.csv")
    va = pd.read_csv(f"{folder}/feedback_validation.csv")
    return tr, va

def prep(df, tok):
    enc = tok(list(df["text"].astype(str)), truncation=True, padding=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="models/feedback_model")
    ...
    print("Feedback model saved to:", args.output_dir)
